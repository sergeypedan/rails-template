class PhoneValidator < ActiveModel::EachValidator
  ...
  def validate_each(record, attribute, value)
    ...
    disallowed_characters = value.sub(/^\+/, "").gsub(/[\s\d\-\(\)]/, "")
    ...
    record.errors[attribute] << "не может быть короче #{MIN_LENGTH} символов" if too_short
    record.errors[attribute] << "не может быть длиннее #{MAX_LENGTH} символов" if too_long
